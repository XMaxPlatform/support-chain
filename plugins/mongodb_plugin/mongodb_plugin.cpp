...
public:
   ...
   static Basetypes::abi xmax_abi; // cached for common use
...
const std::string mongodb_plugin_impl::blocks_col = "Blocks";
const std::string mongodb_plugin_impl::trans_col = "Transactions";
const std::string mongodb_plugin_impl::msgs_col = "Messages";
const std::string mongodb_plugin_impl::accounts_col = "Accounts";


void mongodb_plugin_impl::applied_irreversible_block(const signed_block& block) {
   try {
      if (startup) {
...
void mongodb_plugin_impl::consum_blocks() {
   try {
      signed_block block;
      size_t size = 0;
      while (true) {
...
void mongodb_plugin_impl::process_irreversible_block(const signed_block& block) {
  try {
...
void mongodb_plugin_impl::_process_irreversible_block(const signed_block& block)
{
   ...
   if (processed == 0) {
      if (startup) {
         ...
         // Currently we are creating a 'fake' block in chain_xmax::initialize_chain() since initial accounts
   ...
   for (const auto& thread_vec : block.threads) {
      for (const auto& thread : thread_vec) {
...
// For now providing some simple account processing to maintain xmx_balance
void mongodb_plugin_impl::update_account(const Chain::message_xmax& msg) {
   ...
   if (msg.type == transfer) {
      ...
      asset from_balance = asset::from_string(from_account.view()["xmx_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["xmx_balance"].get_utf8().value.to_string());
      ...
      update_from << "$set" << open_document << "xmx_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "xmx_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
      ...
      asset balance = asset::from_string(from_account.view()["xmx_balance"].get_utf8().value.to_string());
      asset unstack_balance = asset::from_string(from_account.view()["unstacking_balance"].get_utf8().value.to_string());
...
bool mongodb_plugin_impl::is_scope_relevant(const Xmaxplatform::Basetypes::vector<account_name>& scope)
{
...
void mongodb_plugin_impl::init() {
   ...
   if (accounts.count(doc.view()) == 0) {
      ...
      doc << "name" << Config::xmax_contract_name.to_string()
          << "xmx_balance" << asset(Config::initial_token_supply).to_string()
...
void mongodb_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   cfg.add_options()
         ...
         ("queue-size,q", bpo::value<uint32_t>()->default_value(256),
         "The queue size between xmaxrun and MongoDB process thread.")
         ("mongodb-uri,m", bpo::value<std::string>(),
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'XMX' is used if not specified in URI.")
         ;
#endif
}
...
void mongodb_plugin::applied_irreversible_block(const signed_block& block) {
#ifdef MONGODB
...
void mongodb_plugin::plugin_initialize(const variables_map& options)
{
   ...
   if (options.count("mongodb-uri")) {
      ...
      if (options.count("filter-on-accounts")) {
         ...
         auto size = options.at("queue-size").as<uint32_t>();
         my->queue_size = size;
      }
...
void mongodb_plugin::plugin_startup()
{
   ...
   if (my->configured) {
      ...
      // chain_xmax is created and has resynced or replayed if needed
      my->startup = false;
   }
#endif
}
