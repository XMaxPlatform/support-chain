...
DEFINE_INTRINSIC_FUNCTION1(env,sbrk,sbrk,i32,i32,num_bytes) {
   ...
   FC_ASSERT( wasm.current_memory_management != nullptr, "sbrk can only be called during the scope of vm_xmax::vm_call" );
   U32 previous_bytes_allocated = wasm.current_memory_management->sbrk(num_bytes);
   checktime(wasm.current_execution_time(), vm_xmax::get().checktime_limit);
...
DEFINE_INTRINSIC_FUNCTION1(env,malloc,malloc,i32,i32,size) {
   FC_ASSERT( size > 0 );
   int32_t& end = Runtime::memoryRef<int32_t>( Runtime::getDefaultMemory(vm_xmax::get().current_module), 0);
   int32_t old_end = end;
   end += 8*((size+7)/8);
