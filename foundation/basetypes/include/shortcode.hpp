...
namespace ShortCode {
	...
	struct scode_rs
	{
		...
		constexpr scode_rs(sregion r, ssymbol s)
			: region(r)
			, symbol(s)
		{
	...
	static _CONST_EXPR_ sglyph get_sglyph(sregion region, ssymbol sym)
	{		
		switch (region)
		{
		case 0:
	...
	static _CONST_EXPR_ scode_rs get_scode_rs(sglyph ch)
	{
		if ('a' <= ch && ch <= 'z')
		{
			return scode_rs(SR0_NUM, (ssymbol)(ch - 'a' + SR0_SYMBOL_A));
		}
		if ('.' == ch)
		{
			return scode_rs(SR0_NUM, SR0_SYMBOL_POINT);
		}
		if ('1' <= ch && ch <= '4')
		{
			return scode_rs(SR0_NUM, (ssymbol)(ch - '1' + SR0_SYMBOL_1));
		}
		if ('@' == ch)
		{
			return scode_rs(SR1_NUM, SR1_SYMBOL_AT);
		}
		if ('5' <= ch && ch <= '9')
		{
			return scode_rs(SR1_NUM, (ssymbol)(ch - '5' + SR1_SYMBOL_5));
		}
		if ('0' == ch)
		{
			return scode_rs(SR1_NUM, SR1_SYMBOL_0);
		}

		return scode_rs((sregion)SRERROR_NUM, 0);
	}
}