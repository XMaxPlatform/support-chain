...
using std::string;
namespace Xmaxplatform { using Basetypes::simple_symbol_table; }
...
string generate_wren( const Xmaxplatform::Basetypes::struct_t& s, Xmaxplatform::Basetypes::simple_symbol_table& symbols ) {
...
void generate_wren( Xmaxplatform::Basetypes::simple_symbol_table& ss, const char* outfile ) {
  //for( const auto& s : ss.order ) 
}

void generate_hpp( Xmaxplatform::Basetypes::simple_symbol_table& ss, const char* outfile ) {
   struct FakeField { std::string type; std::string name; };
   auto arrays_to_vectors = [](Xmaxplatform::Basetypes::field f) {
   ...
   out << "#pragma once\n";
   out << "namespace Xmaxplatform { namespace Basetypes {\n";
   for( const auto& s : ss.order ) {
   ...
   out << "}} // namespace Xmaxplatform::Basetypes\n";
   ...
   for( const auto& s : ss.order ) {
      ...
      const auto& st = ss.structs[s];
      if( st.base.size() ) {
        out << "FC_REFLECT_DERIVED( Xmaxplatform::Basetypes::" << s << ", (Xmaxplatform::Basetypes::" << st.base <<"), ";
      } else  {
        out << "FC_REFLECT( Xmaxplatform::Basetypes::" <<  std::setw(33) << s << ", ";
      }
      for( const auto& f : st.fields ) {
         out <<"("<<f.name<<")";
      }
...
int  count_fields( Xmaxplatform::Basetypes::simple_symbol_table& ss, const Xmaxplatform::Basetypes::struct_t& st ) {
   if( st.base.size() )
...
int main( int argc, char** argv ) {
  try {
     ...
     Xmaxplatform::Basetypes::simple_symbol_table ss;
     ss.parse(in);
