...
void simple_symbol_table::add_type(const struct_t& s) { try {
      XMAX_ASSERT(is_valid_type_name(s.name), invalid_type_name_exception,
                 "Invalid type name: ${name}", ("name", s.name));
      XMAX_ASSERT(!is_known_type(s.name), duplicate_type_exception,
                 "Duplicate type name: ${name}", ("name", s.name));
      XMAX_ASSERT(s.base.size() == 0 || is_known_type(s.base), unknown_type_exception,
                 "Unknown base type name: ${name}", ("name", s.base));
      for(const auto& f : s.fields) {
         XMAX_ASSERT(is_known_type(f.type), unknown_type_exception,
                    "No such type found: ${type}", ("type",f.type));
         XMAX_ASSERT(is_valid_field_name(f.name), invalid_field_name_exception,
...
void simple_symbol_table::add_type_def(const type_name& from, const type_name& to) { try {
      XMAX_ASSERT(is_valid_type_name(to), invalid_type_name_exception,
                 "Invalid type name: ${name}", ("name", to));
      XMAX_ASSERT(!is_known_type(to), duplicate_type_exception,
                 "Duplicate type name: ${name}", ("name", to));
      XMAX_ASSERT(is_known_type(from), unknown_type_exception,
                 "No such type found: ${type}", ("type", from));
      XMAX_ASSERT(!boost::ends_with(std::string(from), "[]"), type_exception,
...
bool simple_symbol_table::is_known_type(const type_name& n) const { try {
      string name(n);
      XMAX_ASSERT(!name.empty(), invalid_type_name_exception, "Type name cannot be empty");
...
struct_t Xmaxplatform::Basetypes::simple_symbol_table::get_type(type_name name) const {
