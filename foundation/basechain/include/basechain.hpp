...
#ifndef BASECHAIN_NUM_RW_LOCKS
   #define BASECHAIN_NUM_RW_LOCKS 10
#endif

#ifdef BASECHAIN_CHECK_LOCKING
   #define BASECHAIN_REQUIRE_READ_LOCK(m, t) require_read_lock(m, typeid(t).name())
   #define BASECHAIN_REQUIRE_WRITE_LOCK(m, t) require_write_lock(m, typeid(t).name())
#else
   #define BASECHAIN_REQUIRE_READ_LOCK(m, t)
   #define BASECHAIN_REQUIRE_WRITE_LOCK(m, t)
#endif
...
namespace  Basechain {
    ...
    */
   #define BASECHAIN_SET_INDEX_TYPE( OBJECT_TYPE, INDEX_TYPE )  \
   namespace  Basechain { template<> struct get_index_type<OBJECT_TYPE> { typedef INDEX_TYPE type; }; }

   #define BASECHAIN_DEFAULT_CONSTRUCTOR( OBJECT_TYPE ) \
   ...
   class read_write_mutex_manager
   {
      public:
         ...
         {
            _current_lock++;
            new( &_locks[ _current_lock % BASECHAIN_NUM_RW_LOCKS ] ) read_write_mutex();
         }

         read_write_mutex& current_lock()
         {
            return _locks[ _current_lock % BASECHAIN_NUM_RW_LOCKS ];
         }
      ...
      private:
         std::array< read_write_mutex, BASECHAIN_NUM_RW_LOCKS >     _locks;
   ...
   class database
   {
      public:
         ...
         void set_revision( uint64_t revision )
         {
             BASECHAIN_REQUIRE_WRITE_LOCK( "set_revision", uint64_t );
         ...
         {
            BASECHAIN_REQUIRE_WRITE_LOCK("get_mutable_index", typename MultiIndexType::value_type);
            typedef generic_index<MultiIndexType> index_type;
            typedef index_type*                   index_type_ptr;
            assert( _index_map.size() > index_type::value_type::type_id );
            assert( _index_map[index_type::value_type::type_id] );
         ...
         {
             BASECHAIN_REQUIRE_READ_LOCK("find", ObjectType);
         ...
         {
             BASECHAIN_REQUIRE_READ_LOCK("get", ObjectType);
         ...
         {
             BASECHAIN_REQUIRE_WRITE_LOCK("modify", ObjectType);
             typedef typename get_index_type<ObjectType>::type index_type;
         ...
         {
             BASECHAIN_REQUIRE_WRITE_LOCK("remove", ObjectType);
             typedef typename get_index_type<ObjectType>::type index_type;
         ...
         {
             BASECHAIN_REQUIRE_WRITE_LOCK("create", ObjectType);
             typedef typename get_index_type<ObjectType>::type index_type;
   ...
   using shared_multi_index_container = boost::multi_index_container<Object,Args..., Basechain::allocator<Object> >;
}  // namepsace Basechain
