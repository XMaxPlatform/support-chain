...
namespace Baseapp {
   ...
   class application
   {
      public:
         ...
         /**
          * @brief Looks for the --plugin commandline / config option and calls init on those plugins
          *
         ...
         bool                 init(int argc, char** argv) {
            return init_impl(argc, argv, {find_plugin<Plugin>()...});
         }
      ...
      protected:
         ...
         bool init_impl(int argc, char** argv, vector<abstract_plugin*> autostart_plugins);
          ...
          */
         ///@{
         void plugin_inited(abstract_plugin& plug){ initialized_plugins.push_back(&plug); }
   ...
   class plugin : public abstract_plugin {
      public:
         ...
         virtual void init(const variables_map& options) override {
            if(_state == registered) {
               _state = initialized;
               static_cast<Impl*>(this)->plugin_requires([&](auto& plug){ plug.init(options); });
               static_cast<Impl*>(this)->plugin_initialize(options);
               //ilog( "initializing plugin ${name}", ("name",name()) );
               app().plugin_inited(*this);
            }
         ...
         virtual void startup() override {
            ...
            assert(_state == running); // if initial state was not initialized, final state cannot be running
         }
